...
def main():
    ...
    model = vxm.NATfrp_modet.RPNet_diff(img_size)
    model.cuda()
    model_dir = configs['experiments'] + model_folder

    if not os.path.exists('/data/whq/pred/' + configs['dataset_dir_name'] + '/' + model_folder):
        os.makedirs('/data/whq/pred/' + configs['dataset_dir_name'] + '/' + model_folder)
    f = open(configs['dataset_dir_name']+' '+model_folder[:-1]+'.txt', "w")
    ...
    with torch.no_grad():
        ...
        for data in tqdm(test_loader):
            ...
            np.save(
                '/data/whq/pred/' + configs['dataset_dir_name'] + '/' + model_folder + str(stdy_idx) + ',' + pair_name,
                arr=moved)
